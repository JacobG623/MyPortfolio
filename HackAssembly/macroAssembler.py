    ...
    def __init__(self,line):
        ...
        if len(linesplit)>0 and len(linesplit[0]) > 0 :
            if linesplit[0] == '//': #Comment
                ...
                self.data=linesplit[0][1:]
                if len(linesplit) > 1:
                    print("syntax error, spurios data after A instruction on line", linecounter, "in file" , parseFileName)
                    parseError = True
                ...
                if len(linesplit) > 1:
                    print("syntax error, spurios data after C instruction on line", linecounter, "in file" , parseFileName)
                    parseError = True
